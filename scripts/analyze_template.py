...
class TemplateAnalyzer:
    ...
    async def analyze_column_structure(self) -> Dict[str, Any]:
        ...
        key_columns = ["Уровень", "Класс", "Идентификатор", "Имя объекта", "Наименование раздела"]
            ...
            if class_name in self.classes:
                ...
                for class_data in class_attributes:
                    if str(class_data.get("Id")) == str(entity_class.Id):
                        ...
                        if "Attributes" in class_data and class_data["Attributes"]:
                            logger.info(f"Структура атрибутов класса: {json.dumps(dict(list(class_data['Attributes'].items())[:2]), ensure_ascii=False)}")
                ...
                if isinstance(all_attributes_response, dict) and "Result" in all_attributes_response:
                    ...
                    if all_attributes and len(all_attributes) > 1:
                        logger.info(f"Пример атрибута из общего эндпоинта: {json.dumps(all_attributes[0], ensure_ascii=False)}")
            else:
                # Пытаемся найти частичное совпадение
                found = False
                for neosintez_class_name, entity_class in self.classes.items():
                    if class_name.lower() in neosintez_class_name.lower() or neosintez_class_name.lower() in class_name.lower():
                        class_mappings[class_name] = {
                            "id": str(entity_class.Id),
                            "name": entity_class.Name,
...
async def main():
    ...
    try:
        ...
        parser.add_argument("--output", default="template_analysis.json", help="Имя файла для сохранения результатов анализа")
