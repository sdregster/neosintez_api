...
async def debug_hierarchy_import():
    ...
    for level in sorted(preview.objects_by_level.keys()):
        ...
        for obj_data in objects:
            ...
            object_data = {"Name": obj_data["name"], "Entity": {"Id": class_id, "Name": obj_data["class_name"]}}
            ...
            try:
                ...
                try:
                    ...
                    if path_info and hasattr(path_info, "Ancestors") and path_info.Ancestors:
                        ...
                        actual_parent_id = (
                            immediate_parent.Id if hasattr(immediate_parent, "Id") else immediate_parent.get("Id")
                        )
                        actual_parent_name = (
                            immediate_parent.Name if hasattr(immediate_parent, "Name") else immediate_parent.get("Name")
                        )

                        logger.info(
                            f"Проверка через path: родитель объекта - {actual_parent_name} ({actual_parent_id})"
                        )
                        ...
                        if str(actual_parent_id) == str(current_parent_id):
                            ...
                            logger.warning(
                                f"НЕСООТВЕТСТВИЕ: ожидался родитель {current_parent_id}, но получен {actual_parent_id}"
                            )
                    else:
                        ...
                        if created_obj_info:
                            actual_parent = (
                                created_obj_info.get("Parent", {}).get("Id")
                                if isinstance(created_obj_info, dict)
                                else getattr(created_obj_info, "Parent", {}).get("Id")
                            )
                            logger.info(
                                f"Fallback проверка через get_by_id: созданный объект имеет родителя {actual_parent}"
                            )
