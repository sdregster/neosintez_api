...
async def make_api_request(endpoint: str, method: str = "GET", params: Dict = None, data: Any = None) -> Tuple[int, Any]:
    """
        ...
        if class_id:
            ...
            with open(output_dir / "class_attributes_sample.json", "w", encoding="utf-8") as f:
                if isinstance(class_attrs, list) and class_attrs:
                    json.dump(class_attrs[:5], f, ensure_ascii=False, indent=2, cls=UUIDEncoder)
                else:
